%if %defined scl
%ifarch %{arm} %{ix86}
# see note about rust issue 45854 below
...
Patch101:       rust-1.21.0-no-default-pie.patch
...
BuildRequires:  %{?scl_llvm_prefix}cmake3 >= 3.4.3
...
%package -n %{?scl_prefix}rustfmt-preview
...
%endif
Provides:       %{?scl_prefix}rustfmt = %{version}

%description -n %{?scl_prefix}rustfmt-preview
...
%build
%if 0%{?rhel} && 0%{?rhel} < 7
%{?scl:scl enable devtoolset-7 llvm-toolset-7 python27 - << \EOF}
%endif
ulimit -s 6556

# Log some basic hw info to help diagnose failures
(set +e ; ulimit -a ; uname -a ; lscpu ; free ; df -h . ; echo)
...
# but rpmbuild doesn't like having some binaries with and without debuginfo, so disable it all :(
...
%{python} ./x.py build src/tools/rustfmt --verbose
%{python} ./x.py doc
...
rm -f %{buildroot}%{_docdir}/%{pkg_name}/*.old
...
%{_docdir}/%{pkg_name}/html/*.svg
%{_docdir}/%{pkg_name}/html/*.woff
%doc %{_docdir}/%{pkg_name}/html/*.txt


%files -n %{?scl_prefix}rustfmt-preview
%{_bindir}/rustfmt
%{_bindir}/cargo-fmt
...
* Thu Jan 11 2018 Josh Stone <jistone@redhat.com> - 1.23.0-1
- Update to 1.23.0.
- Bootstrap with LLVM 5.

* Tue Dec 12 2017 Josh Stone <jistone@redhat.com> - 1.22.1-1
- Update to 1.22.1.

* Thu Dec 07 2017 Josh Stone <jistone@redhat.com> - 1.21.0-1
- Update to 1.21.0.
